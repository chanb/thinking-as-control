...
class TFAugmentedALEEnv(gym.Env):
    def __init__(self, n_thought_acts=3, n_thought_states=10, d_model=128, max_steps=50, thought_per_state=False):
        super(TFAugmentedALEEnv, self).__init__()
        self.d_model = d_model
        self.base_env = gym.make(
            "ALE/Pacman-v5",
            obs_type="ram",
        ...
        if self.thought_per_state:
            self.initial_thought = nn.Linear(self.obs_dim, self.d_model)
        else:
            self.initial_thought = torch.randn(self.d_model)

        # Thought space
        self.state_embedding = nn.Linear(self.obs_dim, self.d_model)
        self.action_embedding = nn.Embedding(self.n_acts + self.n_thought_acts + 1, self.d_model, padding_idx=0)
        self.lnorm = nn.LayerNorm(d_model)
        self.lnorm.apply(init_weights)
    ...
    def step(self, action):
        ...
        if action < self.n_acts + 1:  # Environment action
            ...
            with torch.inference_mode():
                self.thought = self.transformer(
                    torch.cat((
                        self.state_embedding(torch.tensor(self.env_obs))[None, None],
                        self.action_embedding(torch.tensor(action))[None, None],
                        self.thought[None, None],
                    ), dim=1)
                )[:, -1]
