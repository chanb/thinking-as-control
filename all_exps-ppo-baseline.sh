...
do
    ...
    python thought_gridworld-rl.py --ent_coef=0.02 --num_iterations=${num_iterations} --algo=ppo:reverse_kl --n_thought_states=1 --n_thought_acts=0 --thought_per_state --seed=${seed} --output_file=${results_dir}/no_thought_acts-thought_per_state --log_file=${logs_dir}/no_thought_acts-thought_per_state
done